...
#open up the form for the new client
  ...
  #take form params from new to create new client under the current user
    post "/sessions" do
      if logged_in?
        if !params[:type].empty? && !params[:price].empty? && !params[:date].empty? && !params[:duration].empty? && !params[:rating].empty? && !params[:status].empty? && !params[:location].empty?
          @session = Session.new(:type => params[:type], :price => params[:price], :date => params[:date], :duration => params[:duration], :rating => params[:rating], :status => params[:status], :location => params[:location], :photographer_id => current_user.id)
          @session.save
          redirect "/sessions"
        else
          redirect "/sessions"
  ...
  #take params from edit to update the data of the object
    patch "/sessions/:id" do
      @session = Session.find_by_id(params[:id])
      @session.update(:type => params[:type], :price => params[:price], :date => params[:date], :duration => params[:duration], :rating => params[:rating], :status => params[:status], :location => params[:location], :photographer_id => current_user.id)
      @clients = Client.all
      @session[:client_id] = @artists.last.id
      redirect "/sessions"
    end
